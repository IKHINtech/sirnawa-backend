...
func IplBillDetailRoutes(route fiber.Router) {
	repository := repository.NewIplBillDetailRepository(database.DB)
	services := services.NewIplBillDetailServices(repository, database.DB)
	handlers := handlers.NewIplBillDetailHandler(services)
