...
func IplRateDetailRoutes(route fiber.Router) {
	repository := repository.NewIplRateDetailRepository(database.DB)
	services := services.NewIplRateDetailServices(repository, database.DB)
	handlers := handlers.NewIplRateDetailHandler(services)
